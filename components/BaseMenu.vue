  ...
  <ul class="menu">
    <li v-for="(link, index) in menu" :key="index" class="menu-item actions">
      <router-link v-if="link.path" :to="link.path">
        <span @click="handleMenuClick(link)">{{ link.title }}</span>
      </router-link>
      <a v-else-if="link.href" target="_blank" :href="link.href">
        <span @click="handleMenuClick(link)">{{ link.title }}</span>
      </a>
      <v-dropdown-menu v-else-if="link.sublinks" :dropdown="link" />
      <span v-else @click="handleMenuClick(link)">{{ link.title }}</span>
    </li>
  </ul>
</template>
...
export default {
  ...
  methods: {
    handleMenuClick(link) {
      this.$emit('handleMenuClick', { link })
    }
  }
}
</script>
